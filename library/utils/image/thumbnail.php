...
//    global $DOCUMENT_ROOT;
  ...
  function getThumbData( $data )
    {
    $real_width = $this->GetImageData('width');
    $w_ratio = $this->max_width  / $real_width;
    if($w_ratio > 1) { $this->max_width = $real_width; $w_ratio = 1; }
    $real_height = $this->GetImageData('height');
    $h_ratio = $this->max_height / $real_height;
    if($h_ratio > 1) { $this->max_height = $real_height; $h_ratio = 1 ;}
  ...
  function Create( $dest="" )
    {
    $new_width = $this->GetThumbData('width');
    $new_height = $this->GetThumbData('height');
    $img_des = ImageCreateTrueColor ($new_width ,  $new_height);
    //$img_des = ImageCreate ($new_width ,  $new_height);
    ...
    ImageCopyResized( $img_des, $img_src, 0, 0, 0, 0, $new_width, $new_height, $this->GetImageData('width'), $this->GetImageData('height') );
